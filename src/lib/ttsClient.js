...
// голоса не знают и продолжают ходить за прежним аудио. Экспортируется как
// ЗАПАСНОЙ голос: чем угадывать, когда нужного не нашлось, лучше сказать
// основным — озвучка должна деградировать, а не отказывать.
export const PRIMARY_VOICE = "primary";
const DEFAULT_VOICE = PRIMARY_VOICE;

/**
 * Режет длинный текст на куски не длиннее max — по ОДНОМУ запросу озвучки на
 * кусок. Плеер играет их подряд по общей шкале времени, так что для слушателя
 * это одна дорожка.
 *
 * ЗАЧЕМ. Предел длины — предохранитель от случайного сжигания квоты, и снимать
 * его нельзя: реплика диалога бывает любой длины, порогом её не ограничить (у
 * Google предел выше нашего, но он тоже конечен). А отказ был полным: длинная
 * реплика роняла ВЕСЬ диалог одним «Этот текст не получается озвучить». Резать
 * дешевле и честнее — предохранитель на месте, а озвучка есть.
 *
 * Границы выбираем по убыванию естественности: конец предложения → конец
 * оборота (запятая, тире, двоеточие) → любой пробел → жёсткий рез по пределу
 * (текст без пробелов вообще). Пауза между кусками совпадает с той, что человек
 * и так услышал бы на этой границе.
 *
 * Разбиение ДЕТЕРМИНИРОВАНО: тот же текст всегда даёт те же куски. Это не
 * косметика — кусок это ключ в общем кэше озвучки, и «плавающее» разбиение
 * заставляло бы платить за одну и ту же фразу заново. Короткий текст (а это
 * всё, кроме реплик диалога) возвращается ОДНИМ куском как есть, поэтому
 * накопленный кэш карточек, примеров и чтения остаётся действительным.
 */
export function splitForTts(text, max = MAX_TTS_TEXT_LEN) {
  ...
  while (rest.length > max) {
    // Плюс один символ: он показывает, годится ли сама граница на пределе.
    const at = lastBreak(rest.slice(0, max + 1));
    const cut = at > 0 ? at : max; // границы нет вовсе — режем ровно по пределу
...
const SENTENCE_END = /[.!?…]/;
const CLAUSE_END = /[,;:—–)]/;
// Закрывающая кавычка/скобка после знака: «…конец.» — знак всё равно там.
const CLOSER = /["»”'’)\]]/;

// Последняя подходящая граница в куске: сначала конец предложения, потом конец
// оборота, потом любой пробел. Возвращает индекс, ПО который резать (сам пробел
// в кусок не попадает), либо -1, если границ нет вовсе.
function lastBreak(chunk) {
  const endsWith = (i, re) => {
    let j = i - 1;
    if (j > 0 && CLOSER.test(chunk[j])) j -= 1;
    return j >= 0 && re.test(chunk[j]);
  };
  for (const re of [SENTENCE_END, CLAUSE_END, null]) {
    for (let i = chunk.length - 1; i > 0; i -= 1) {
      if (!/\s/.test(chunk[i])) continue;
      if (!re || endsWith(i, re)) return i;
    }
  }
  return -1;
}
 ...
 *   empty        — озвучивать нечего: пустой текст (или не задан язык). Раньше
 *                  это и «слишком длинно» были ОДНОЙ причиной «unavailable», и
 *                  экран отвечал «Этот текст не получается озвучить» — фраза,
 *                  из которой нельзя понять ни что случилось, ни что делать;
 *   tooLong      — текст длиннее предела ОДНОГО запроса. Плеер до этой причины
 *                  не доходит: длинное он режет сам (см. splitForTts). Причина
 *                  осталась для тех, кто просит озвучку одним куском;
...
export async function fetchTtsUrlResult({
  ...
  const clean = String(text ?? "").trim();
  if (!clean || !learnLang) return { url: null, reason: "empty" };
  if (clean.length > MAX_TTS_TEXT_LEN) {
    // Длину называем прямо и с числами: раньше это и пустой текст возвращали
    // одну безымянную причину, и по экрану их было не отличить.
    // eslint-disable-next-line no-console
    console.warn(
      `[tts] текст длиннее предела: ${clean.length} > ${MAX_TTS_TEXT_LEN}, ${learnLang}/${voice} — запрос не отправлен`,
    );
    return { url: null, reason: "tooLong", params: { max: MAX_TTS_TEXT_LEN } };
  }
