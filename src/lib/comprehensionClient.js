...
const DIALOGUES_KEY = "listeningDialogues"; // { "de-ru|mixed|v2d2": [ {…диалог+вопросы}, … ] }
const QUESTIONS_KEY = "readingQuestions"; // { "<hash>": { questions: [] } }

// ВЕРСИЯ ВОПРОСОВ НА ПОНИМАНИЕ. Подмешивается в ОБА ключа — и к тексту чтения, и
// к набору диалога, — поэтому при её смене кэш просто не находит старых записей
// и вопросы генерируются заново, уже по новым правилам.
//
// ЗАЧЕМ. Вопросы кэшируются вместе с источником и живут долго: в аудировании при
// заходе на экран показывается последний диалог из истории, пока человек сам не
// нажмёт «Новый диалог», — без версии старые вопросы висели бы месяцами, и
// правка механизма до людей просто не дошла бы.
//
// КОГДА ПОДНИМАТЬ: правка меняет ТРЕБОВАНИЯ к вопросам (промпт или фильтры), и
// прежние наборы по новым правилам уже не годятся. Правка вида (вёрстка,
// подписи кнопок) версию не трогает — незачем терять готовые вопросы.
//
// v2 — запрет дословности: утверждения перефразируются, а списанные с источника
// бракуются фильтром (см. lib/comprehension.js).
//
// Старые записи после смены версии осиротеют и вытеснятся существующими
// лимитами (MAX_QUESTION_ENTRIES / MAX_DIALOGUES_PER_PAIR) — отдельная чистка
// не нужна, механика кэша и сами лимиты не меняются.
const QUESTIONS_VERSION = 2;

// ВЕРСИЯ ДИАЛОГА — отдельно от версии вопросов и только в ключе диалогов.
// Поднимается, когда меняются требования к САМОМУ диалогу, а вопросы к текстам
// чтения при этом ни при чём и терять их незачем.
//
// v2 — у каждой реплики появился пол говорящего: по нему выбирается голос
// озвучки (см. lib/tts.js). В диалогах старее этой версии пола нет, а текст
// написан без оглядки на голоса — там и живёт та самая рассогласованность,
// из-за которой мужской голос говорил о себе «я пришла». Такие наборы просто
// не находятся в кэше, и экран берёт новый диалог — уже с полами.
const DIALOGUE_VERSION = 2;
...
// Ключ набора: пара + источник слов + версия вопросов + версия диалога. Обе
// версии в ключе — потому что вопросы кэшируются ВМЕСТЕ с диалогом: набор
// старой версии (хоть по вопросам, хоть по самому диалогу) просто не находится,
// и экран берёт новый диалог с новыми вопросами.
function dialogueKey(pairKey, source = "mixed") {
  return `${pairKey}|${source}|v${QUESTIONS_VERSION}d${DIALOGUE_VERSION}`;
}
...
/**
 * Ключ кэша вопросов = хеш(версия + язык + сам текст): тот же текст — те же
 * вопросы, но только в пределах одной версии вопросов. Смена версии меняет хеш,
 * поэтому к уже прочитанному тексту вопросы соберутся заново по новым правилам.
 */
export function questionsKeyFor(learnLang, sentences) {
  const passage = (Array.isArray(sentences) ? sentences : [])
    .map((s) => s.text)
    .join(" ");
  return hashKey(`v${QUESTIONS_VERSION}|${learnLang}|${passage}`);
}
