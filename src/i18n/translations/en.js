...
export default {
  ...
  whatsnew: {
    ...
    entries: {
      ...
      setupAfterConfirm: {
        title: "Setup and intro right after you confirm your email",
        desc: "A new user who followed the link from the confirmation email landed straight in the app: no choice of languages, topic and level, no short intro — just a \"Welcome\" greeting. The cause was the device, not the person: a link from an email creates a session for the new account in the very browser that still held the previous account's settings, and this path never goes through a sign-out, which is what clears them. The app saw someone else's language, topic and level, decided the newcomer was already set up, and skipped both the onboarding and the intro. Now the device is wiped completely when it changes hands, and the \"intro seen\" mark belongs to the account rather than to the browser. The order is the same for every way in — password, straight after signing up, and the confirmation link: setup first, then the four intro screens, then everything else. Anyone already set up who has seen the intro still goes straight into the app, and the updates window no longer appears on top of the new-password screen.",
      },
