...
-- ============================================================================
-- Свои темы пользователя (пресеты в коде, свои — по языковой паре)
-- ----------------------------------------------------------------------------
-- Свои темы генерации живут в самой строке пары: массив коротких названий
-- (до 3 слов каждое, максимум 3 штуки на пару — лимит держит клиент). Хранение
-- в user_languages даёт бесплатно и переезд между устройствами (грузится вместе
-- со списком языков), и раздельность по языкам: у немецкого свои темы, у
-- греческого свои. Пресеты в этот массив НЕ входят — они всегда в коде и
-- неудаляемы. RLS на таблице уже настроен (политики *_own выше). Идемпотентно.
alter table public.user_languages
...
-- ============================================================================
-- Движок заданий · Регулируемая нагрузка занятия (profiles.session_load)
-- ----------------------------------------------------------------------------
-- Приложение само собирает занятие на сегодня из доступных форматов. Стартовый
-- объём считается автоматически, дальше пользователь правит его ползунком —
-- выбор хранится здесь, чтобы жить между устройствами. RLS profiles уже покрывает
-- (политики *_own выше) — новых политик не нужно. Идемпотентно.
--   session_load — 'light' | 'normal' | 'heavy' | 'auto' (по умолчанию 'auto':
--                  объём считается из daily_new_limit и числа созревших повторений).
-- ============================================================================

alter table public.profiles
  add column if not exists session_load text not null default 'auto';
