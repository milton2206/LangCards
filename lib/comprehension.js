...
- Для каждого дай "explanation" на языке ${native}: почему ответ именно такой, СО ССЫЛКОЙ на то, что было сказано в ${sourceNoun} (перескажи или процитируй нужное место). Это объяснение показывается пользователю, когда он ошибается.`;
}

// Схема одного вопроса в JSON — тоже общая.
function questionSchema(learn, native) {
  return `{ "statement": "утверждение о содержании на ${learn}", "statementTranslation": "перевод утверждения на ${native}", "answer": true, "explanation": "почему ответ такой, со ссылкой на содержание, на ${native}" }`;
}
...
export function buildDialoguePrompt({
  ...
  // Диалог собираем вокруг слов пользователя по тому же правилу источника, что и
  // текст чтения (6.1) — одна семантика mine/mixed/new (см. wordSourceGoal.js).
...
export async function generateDialogueComprehension(params) {
  ...
  if (dialogue.length === 0 || list.length === 0) {
    throw new Error("Не удалось составить диалог. Попробуйте ещё раз.");
  }
...
export async function generateTextComprehension(params) {
  ...
  const list = normalizeQuestions(raw.questions, questions);
  if (list.length === 0) {
    throw new Error("Не удалось составить вопросы к этому тексту. Попробуйте ещё раз.");
  }
