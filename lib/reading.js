...
export function buildReadingPrompt({
  ...
  if (effectiveSource === "mine") {
    // Только знакомые слова — закрепление. Повтор допустим и даже желателен.
    goalBlock = `ГЛАВНОЕ — только знакомые слова (закрепление):
- Строй текст ПОЧТИ ЦЕЛИКОМ из слов пользователя (список ниже). Незнакомых слов избегай — допустимы лишь короткие служебные слова (артикли, предлоги, союзы) для связности.
- Повторять знакомые слова — нормально: цель в том, чтобы закрепить уже взятое. Форму слова меняй свободно (падеж, число, время).

Слова пользователя (используй ТОЛЬКО их):
${knownList}`;
