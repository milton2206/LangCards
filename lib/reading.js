...
export function buildPhrasePrompt({
  ...
  return `Переведи ФРАГМЕНТ предложения на родной язык пользователя.
...
/**
 * Перевод фрагмента предложения. Возвращает { translation }.
...
export async function translatePhrase(params) {
  ...
  const raw = await requestModelJson(
    async () => {
      const message = await client().messages.create({
        model: MODEL,
        max_tokens: 200, // один перевод — короткий потолок
        system:
