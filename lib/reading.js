...
import { buildWordSourceGoal } from "./wordSourceGoal.js";
...
// Сколько заголовков недавних текстов отдаём модели «чтобы не повторяться» —
// предохранитель стоимости: список коротких заголовков, не самих текстов.
...
export function buildReadingPrompt({
  ...
  // Семантика источника единая для чтения и аудирования — см. wordSourceGoal.js.
...
- Дай к нему "options": 4 РЕАЛЬНЫХ слова языка ${learn} ТОЙ ЖЕ части речи и формы. Ровно одно из них совпадает с "blank" (верное), остальные три правдоподобны на месте пропуска, но по смыслу предложения не подходят. Никаких выдуманных слов.\n`
  ...
  // сценарий (тема «работа» → каждый раз «первый день на работе»). Даём модели
  // заголовки недавних текстов пользователя и просим ДРУГОЙ сценарий в рамках
  // темы. Это только заголовки (не сами тексты) — дёшево. Пустой список — блока
  // нет (промпт остаётся прежним, как без этой фичи).
  const recent = (Array.isArray(recentTitles) ? recentTitles : [])
    .map((s) => String(s ?? "").trim().replace(/\s+/g, " ").slice(0, 80))
    .filter(Boolean)
    .slice(0, MAX_RECENT_TITLES);
  const diversityBlock = recent.length
    ? `\nНЕ ПОВТОРЯЙ НЕДАВНИЕ СЮЖЕТЫ (важно для разнообразия):
- В этой теме у пользователя уже были такие тексты (заголовки): ${recent.map((r) => `«${r}»`).join(", ")}.
- Придумай ДРУГОЙ сценарий в рамках той же темы: иное место, иная ситуация, иные герои, иная завязка. Например, тема «работа» — это не только первый день, но и собеседование, увольнение, конфликт с коллегой, повышение, совещание, командировка.
