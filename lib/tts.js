...
function resolveVoice(langVoices, voiceId) {
  ...
  // Совсем пустой список — падать некуда, но и озвучку ронять не за что: пусть
  // говорит основной голос языка. Правило одно на всю цепочку: не нашли голос —
  // берём основной, а не отказываем.
  const base = pool[0] || langVoices[0];
  if (!base) return null;
...
export async function getAccessToken() {
  ...
  if (!res.ok) {
    // В логах Vercel от этого места раньше оставался голый 502 — неотличимый от
    // сбоя синтеза и от сбоя записи в кэш. Пишем, ЧТО именно ответил Google.
    const detail = await res.text().catch(() => "");
    // eslint-disable-next-line no-console
    console.warn(
      `[tts] OAuth Google не прошёл: HTTP ${res.status}`,
      detail.slice(0, 300),
    );
 ...
 *
 * У ошибок 502 РАЗНЫЕ коды — по звену, которое упало. Раньше все три отдавали
 * голый 502, и по логам Vercel нельзя было понять, что именно сломалось:
 *   ttsAuth    — не получен токен сервисного аккаунта Google;
 *   ttsSynth   — Google не отдал аудио (ошибка или пустой ответ);
 *   ttsStorage — аудио получено, но не записалось в общий кэш. Этот случай в
 *                метриках Google выглядит успешным запросом — только по коду
 *                и по строке в логах его и видно.
 * Каждое из мест дополнительно пишет console.warn с деталями (см. ниже).
 *
...
export async function getOrCreateSpeech({
  ...
  if (!entry) {
    const err = new Error(`Озвучка для языка «${learnLang}» не настроена.`);
    err.status = 400;
    throw err;
  }
  ...
  if (!synthRes.ok) {
    ...
    // eslint-disable-next-line no-console
    console.warn(
      `[tts] синтез не удался: HTTP ${synthRes.status}, ${learnLang}/${name}, ${clean.length} симв.`,
      detail.slice(0, 300),
    );
  ...
  if (!audioContent) {
    // Google ответил 200, но без аудио. Раньше это падало дальше по коду
    // невнятной ошибкой Buffer — теперь это честный сбой синтеза.
    // eslint-disable-next-line no-console
    console.warn(`[tts] Google вернул 200 без аудио: ${learnLang}/${name}`);
    const err = new Error("Google TTS вернул пустой ответ.");
    err.status = 502;
    err.code = "ttsSynth";
    throw err;
  }
  ...
  if (error) {
    // САМЫЙ НЕЗАМЕТНЫЙ случай: у Google запрос прошёл успешно и в его метриках
    // он чистый, а до человека аудио не доехало. Отдельный код и строка в логах
    // существуют именно ради него — иначе это звено вообще ничем не отличить.
    // eslint-disable-next-line no-console
    console.warn(
      `[tts] запись в кэш не удалась: ${path}, ${bytes.length} Б —`,
      error.message,
    );
