...
function cachePath(learnLang, text, rate) {
  const hash = crypto.createHash("sha256").update(text).digest("hex");
  if (rate === DEFAULT_TTS_RATE) return `${learnLang}/${hash}.mp3`;
  const folder = `r${rate.toFixed(2).replace(".", "")}`; // 0.7 → r070
  return `${learnLang}/${folder}/${hash}.mp3`;
}
 ...
 */
export async function getOrCreateSpeech({ text, learnLang, rate, onSynthesize }) {
  ...
  const path = cachePath(learnLang, clean, speakingRate);
  ...
  try {
    ...
    if (head.ok) return { url: publicUrl, cached: true, rate: speakingRate };
  } catch {
  ...
  return { url: publicUrl, cached: false, rate: speakingRate };
}
